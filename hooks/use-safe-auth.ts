...
export function useSafeAuth() {
    ...
    // Intentar obtener el contexto de auth de forma segura
    ...
    useEffect(() => {
        try {
            ...
            console.warn("AuthProvider context no está disponible:", error)
    ...
    const signInWithErrorHandling = async () => {
        if (!auth) {
            setAuthError({
                message: "El sistema de autenticación no está disponible. Recarga la página.",
                type: "general",
                timestamp: new Date(),
            })
            return
    ...
    const isTokenExpiringSoon = (minutesThreshold = 5) => {
        try {
            ...
            const expirationTime = auth.user.expires_at * 1000
    ...
    return {
        // Estado original de auth (con valores por defecto si no está disponible)
